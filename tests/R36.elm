module R36 exposing (suite)

import Array
import Expect
import Fuzz exposing (array, bool, float, floatRange, int, intRange, list, percentage, string, tuple, tuple3)
import P36 exposing (solve)
import Set
import Test exposing (Test, describe, fuzz, test)


primeFactorsUntil100 : List ( Int, List Int )
primeFactorsUntil100 =
...
adaptTestCases : List ( Int, List Int ) -> List ( Int, List ( Int, Int ) )
adaptTestCases l =
    List.map
        (\( n, ll ) ->
            ( n
...
listToString : List ( Int, Int ) -> String
listToString l =
    List.map (\( a, b ) -> "(" ++ String.fromInt a ++ " " ++ String.fromInt b ++ ")") l
        |> String.join ","
...
suite =
    describe "P36"
        (List.map
            (\( input, out ) ->
                test ("Prime factors " ++ String.fromInt input ++ " = " ++ listToString out) <|
                    \_ -> Expect.equal (solve input) out
            )
            (adaptTestCases testCases)
        )
